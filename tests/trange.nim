...
test "Range-like Nim procedure":
  ...
  check toSeq(range(5)) == @[0, 1, 2, 3, 4]
  checkpoint "Two arguments - start and stop"
  check toSeq(range(3, 5)) == @[3, 4]
  checkpoint "Negative start and positive stop"
  check toSeq(range(-7, 3)) == @[-7, -6, -5, -4, -3, -2, -1, 0, 1, 2]
  checkpoint "3 positive arguments"
  check toSeq(range(1, 10, 3)) == @[1, 4, 7]
  checkpoint "Positive start, negative stop and step"
  check toSeq(range(0, -10, -2)) == @[0, -2, -4, -6, -8]
  check toSeq(range(5, -5, -3)) == @[5, 2, -1, -4]
  checkpoint "Variables"
  const a = 10
  check toSeq(range(a, a + 2)) == @[a, a + 1]
  checkpoint "Zero step"
  expect ValueError:
    discard toSeq(range(1, 2, 0))
  checkpoint "For loop"
  var data: seq[int] = @[]
